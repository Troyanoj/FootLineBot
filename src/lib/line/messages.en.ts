...
export const registrationSuccessMessage = (event: Event): string => {
  ...
  const time = new Date(dateTimeString).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: false });
...
export const groupNotFoundMessage = (groupId?: string): string => {
  let msg = `❌ *Group not found*\n\n`;
  msg += `No group found with ID: ${groupId || 'not specified'}\n\n`;
  msg += `📋 *Solution:*\n`;
  msg += `1. Ask the group admin to execute !register or !setup in the LINE group\n`;
  msg += `2. Then use !join [ID] again\n\n`;
  msg += `Use !groups to see available groups.`;
  return msg;
};
...
export const lineupMessage = (
  ...
  // Note about viewing full list - we could enhance this later with actual URL
