...
function fixFile(filePath) {
    ...
    // 1. Remove ANY existing handlePosicion if present (cleanup)
    content = content.replace(/export async function handlePosicion[^{]+{[\\s\\S]+?}\\n/g, '');
    
    // 2. Remove ANY existing handleUserCommand including its comments
    content = content.replace(/\\/\\*\\*[\\s\\S]+?export async function handleUserCommand[\\s\\S]+$/, '');
    
    // 3. Append EVERYTHING to the end of the file safely
...
fixFile('src/lib/line/handlers/user.handlers.ts');
